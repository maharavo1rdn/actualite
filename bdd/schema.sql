-- 1.(Géopolitique, Humanitaire, etc.)
...
CREATE TABLE articles (
    ...
    slug VARCHAR(255) UNIQUE NOT NULL,-- Pour des URLs propres (ex: /guerre-iran-impact)
...
-- 3.(Officiel, Agence, Social Media)
...
-- Pour accélérer l'affichage de la page d'accueil (tri par date)
CREATE INDEX idx_articles_date_pub ON articles(date_publication);

-- Pour accélérer l'accès à un article via son URL (SEO)
CREATE INDEX idx_articles_slug ON articles(slug);

-- Pour accélérer les filtres par thématique (ex: menu "Guerre")
CREATE INDEX idx_articles_cat ON articles(id_categorie);

-- Pour le fil d'actualité en temps réel (Timeline)
CREATE INDEX idx_chrono_date ON evenements_chronologie(date_evenement);

-- Pour sécuriser et accélérer la connexion des rédacteurs
