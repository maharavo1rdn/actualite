...
<body class="bg-slate-100 text-slate-900">
    ...
    <main class="max-w-7xl mx-auto px-5 py-8">
        ...
        <section class="mb-6 max-w-4xl">
            ...
            <!-- FIX 2: text-slate-400 → text-slate-600 (contrast ~5.9:1 on slate-100 background) -->
            <p class="mono text-sm text-slate-600">
                Publié le <?= htmlspecialchars(formatDateArticleFr($article['date_publication'])) ?> &nbsp;·&nbsp; La Rédaction
            </p>
        </section>

        <!-- ── COVER IMAGE ───────────────────── -->
        <!-- FIX 3: added id="carousel-wrapper" so JS can find the element for swipe listeners -->
        <section id="carousel-wrapper" class="bg-white border border-slate-200 rounded-2xl overflow-hidden mb-7 relative" style="position: relative;">
                    ...
                    <?php foreach ($allImages as $i => $img): ?>
                        ...
                        <div class="carousel-slide <?= $i === 0 ? '' : 'hidden' ?>" data-index="<?= $i ?>" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;">
                            <img
                                ...
                                alt="<?= htmlspecialchars(!empty($img['legende']) ? $img['legende'] : 'Image ' . ($i + 1) . ' — ' . strip_tags($article['titre'])) ?>"
            ...
            <?php else: ?>
                <!-- FIX 1: added descriptive alt attribute to the single-image path -->
                <div style="width: 100%; height: 450px; position: relative;">
                    <img
                        src="<?= htmlspecialchars($coverImage) ?>"
                        alt="<?= htmlspecialchars(!empty($primaryImage['legende']) ? $primaryImage['legende'] : 'Image de couverture — ' . strip_tags($article['titre'])) ?>"
                        style="width: 100%; height: 100%; object-fit: cover;">
        ...
        <section class="bg-white border border-slate-200 rounded-2xl p-6 md:p-8">
            <h2 class="text-lg font-bold text-slate-900 mb-5">
                Sources vérifiées
                <!-- FIX 2: text-slate-400 → text-slate-600 (contrast ~5.9:1 on white) -->
                <span class="mono text-sm font-normal text-slate-600 ml-2"><?= count($sources) ?> source<?= count($sources) !== 1 ? 's' : '' ?></span>
            </h2>
    ...
    <script>
        (function () {
            ...
            // FIX 3a: guard — if no slides exist (single-image path) do nothing at all
            if (total === 0) return;
            ...
            // FIX 3b: now safe — carousel-wrapper id exists on the <section> above
