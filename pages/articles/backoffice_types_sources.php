<?php
/**
 * Backoffice — Référentiel Types de sources
 * URL : /backoffice/types-sources
 */
session_start();
...
$result = $controller->listTypeSourcesPaginated($perPage, $offset, $q);
$rows = $result['rows'];
$total = $result['total'];
...
$editType = null;
if ($editId > 0) {
    $all = $controller->getAllTypeSources();
    foreach ($all as $t) {
        if ((int) $t['id'] === $editId) {
            $editType = $t;
            break;
...
function buildTypeSourcePagerUrl(int $p): string
{
    $params = array_filter([
        'q' => $_GET['q'] ?? '',
        'page' => $p > 1 ? $p : '',
    ]);
    return '/backoffice/types-sources' . ($params ? '?' . http_build_query($params) : '');
}
?>
<!DOCTYPE html>
<html lang="fr">
...
<header class="bg-black text-white sticky top-0 z-10">
    <div class="container mx-auto px-6 h-14 flex items-center justify-between gap-4">
        <span class="mono text-sm tracking-tight">Info Iran / <span class="text-gray-400">types sources</span></span>
...
<main class="container mx-auto px-6 py-10 max-w-[90rem]">
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <h1 class="text-3xl font-semibold text-gray-900 tracking-tight">Gestion des types de sources</h1>
            <p class="mono text-sm text-gray-400 mt-1">
                <?= $total ?> type<?= $total > 1 ? 's' : '' ?>
                <?php if ($q !== ''): ?>
                    · <a href="/backoffice/types-sources" class="text-red-400 hover:text-red-500 transition-colors">✕ effacer filtre</a>
                <?php endif; ?>
            </p>
        </div>
    </div>
    ...
    <div class="lg:grid lg:grid-cols-[1fr_340px] gap-6 items-start">
        ...
        <section class="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <form method="GET" action="/backoffice/types-sources" class="p-5 border-b border-gray-200 bg-gray-50/40 flex gap-3 items-end">
                <div class="flex-1 min-w-[180px]">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Recherche</label>
                    <input type="text" name="q" value="<?= htmlspecialchars($q) ?>" placeholder="Libellé de type..."
            ...
            <?php if (empty($rows)): ?>
                <div class="px-6 py-20 text-center mono text-base text-gray-400">Aucun type de source trouvé.</div>
            <?php else: ?>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200 mono text-xs text-gray-400 uppercase tracking-widest">
                                <th class="px-6 py-3 font-normal">Libellé</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php foreach ($rows as $i => $row): ?>
                                <tr class="hover:bg-blue-50/30 transition-colors <?= $i % 2 === 1 ? 'bg-gray-50/60' : 'bg-white' ?>">
                                    <td class="px-6 py-4">
                                        <span class="mono text-xs bg-slate-100 text-slate-700 border border-slate-200 px-2.5 py-1 rounded-md">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            <a href="/backoffice/types-sources?edit=<?= (int) $row['id'] ?><?= $q !== '' ? '&q=' . urlencode($q) : '' ?>" class="mono text-sm text-gray-600 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg transition-colors">Modifier</a>
                                            <form method="POST" action="/backoffice/gestion/traitement" onsubmit="return confirm('Supprimer ce type de source ?');">
                                                <input type="hidden" name="action" value="type_source_delete">
            ...
            <?php if ($totalPages > 1): ?>
                <div class="flex items-center gap-1.5 mt-6 p-5 border-t border-gray-200">
                    <a href="<?= buildTypeSourcePagerUrl($page - 1) ?>" class="mono px-3 py-1.5 border border-gray-200 rounded-lg text-sm transition-colors <?= $page <= 1 ? 'opacity-30 pointer-events-none' : 'hover:bg-gray-50 text-gray-700' ?>">‹</a>
                    <?php for ($p = max(1, $page - 2); $p <= min($totalPages, $page + 2); $p++): ?>
                        <a href="<?= buildTypeSourcePagerUrl($p) ?>" class="mono px-3 py-1.5 border rounded-lg text-sm transition-colors <?= $p === $page ? 'bg-black text-white border-black' : 'border-gray-200 hover:bg-gray-50 text-gray-700' ?>">
                            <?= $p ?>
                        </a>
                    <?php endfor; ?>
                    <a href="<?= buildTypeSourcePagerUrl($page + 1) ?>" class="mono px-3 py-1.5 border border-gray-200 rounded-lg text-sm transition-colors <?= $page >= $totalPages ? 'opacity-30 pointer-events-none' : 'hover:bg-gray-50 text-gray-700' ?>">›</a>
                </div>
            <?php endif; ?>
        </section>
        ...
        <aside class="sticky top-20 bg-white border border-gray-200 rounded-xl p-5">
            <?php if (!$editType): ?>
                <h2 class="text-lg font-semibold text-gray-900 mb-4">Nouveau type</h2>
                <form method="POST" action="/backoffice/gestion/traitement" class="space-y-4">
                    <input type="hidden" name="action" value="type_source_create">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Libellé <span class="mono text-xs text-red-400">*</span></label>
                        <input type="text" name="libelle" required placeholder="OFFICIEL"
            ...
            <?php else: ?>
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-semibold text-gray-900">Éditer le type</h2>
                    <a href="/backoffice/types-sources" class="mono text-sm text-gray-500 hover:text-gray-700 transition-colors">✕</a>
                </div>
                <form method="POST" action="/backoffice/gestion/traitement" class="space-y-4">
                    <input type="hidden" name="action" value="type_source_update">
                    <input type="hidden" name="id" value="<?= (int) $editType['id'] ?>">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Libellé <span class="mono text-xs text-red-400">*</span></label>
                        <input type="text" name="libelle" required value="<?= htmlspecialchars($editType['libelle']) ?>"
                               class="w-full mono text-sm px-3.5 py-2.5 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:border-gray-400 focus:outline-none transition-colors">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                        <textarea name="description" rows="4" class="w-full mono text-sm px-3.5 py-2.5 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:border-gray-400 focus:outline-none transition-colors"><?= htmlspecialchars((string) ($editType['description'] ?? '')) ?></textarea>
                    </div>
